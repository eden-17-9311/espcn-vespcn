...
def main() -> None:
    ...
    for index in range(total_files):
        ...
        lr_multi_frame = torch.from_numpy(lr_multi_frame).unsqueeze(0).to(config.device)
        # ==================== Early Fusion 结束 ====================

        # Only reconstruct the Y channel image data.
        with torch.no_grad():
            sr_y_tensor = g_model(lr_multi_frame)

        # --- 新增/修改开始: 尺寸对齐 ---
        # 获取超分后的实际尺寸 (Height, Width)
        sr_h, sr_w = sr_y_tensor.shape[-2:]

        # 1. 调整 GT Tensor 尺寸 (用于计算 PSNR/SSIM)
        # 如果尺寸不一致，使用插值调整 GT 到 SR 的尺寸
        if gt_y_tensor.shape[-2:] != (sr_h, sr_w):
            gt_y_tensor = torch.nn.functional.interpolate(
                gt_y_tensor, size=(sr_h, sr_w), mode='bilinear', align_corners=False)

        # 2. 调整 Cb/Cr 通道尺寸 (用于保存图片 cv2.merge)
        # 如果尺寸不一致，使用 OpenCV 调整尺寸
        if gt_cb_image.shape[:2] != (sr_h, sr_w):
            gt_cb_image = cv2.resize(gt_cb_image, (sr_w, sr_h), interpolation=cv2.INTER_CUBIC)
            gt_cr_image = cv2.resize(gt_cr_image, (sr_w, sr_h), interpolation=cv2.INTER_CUBIC)
        # --- 新增/修改结束 ---

        # Save image
        sr_y_image = imgproc.tensor_to_image(sr_y_tensor, range_norm=False, half=True)
        sr_y_image = sr_y_image.astype(np.float32) / 255.0
        sr_ycbcr_image = cv2.merge([sr_y_image, gt_cb_image, gt_cr_image]) # 现在这里尺寸一致了，不会报错
        sr_image = imgproc.ycbcr_to_bgr(sr_ycbcr_image)
        cv2.imwrite(sr_image_path, (sr_image * 255.0).astype(np.uint8)) # 顺便修复一下之前的类型警告
